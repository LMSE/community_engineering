...
class DyMMMBatchSimulator:
    """
    A class to simulate batch processes for DyMMM (Dynamic Multi-Model Metabolic analysis).

    Attributes:
        columnNames (list): List of column names in the data frame.
        CHUNKSIZE (int): Size of each chunk of data to be processed.
        CPU_COUNT (int): Number of CPU cores to be used for parallel processing.
    ...
    def process_frame(self, id, df):
        """
        Processes a single frame of data by running the DyMMM simulation and analysis.

        Args:
            id (int): The identifier for the frame being processed.
            df (DataFrame): The data frame containing the simulation parameters for this frame.
        """
    ...
    def isSteadyState(self, df, colName):
        """
        Checks if the simulation has reached a steady state.

        Args:
            df (DataFrame): The data frame containing simulation results.
            colName (str): The name of the column to check for steady state.

        Returns:
            bool: True if the simulation is in steady state, False otherwise.
        """
    ...
    def runSimulation(self, paramsFile):
        """
        Runs the simulation sequentially for all pending simulations.
    ...
    def runSimulationParallel(self, paramsFile):
        """
        Runs the simulation in parallel across multiple processors.
...
if __name__ == '__main__':
    ...
    if paramDataFile == None:
        print("DyMMMBatchSimulator2:nothing to simulate")
        exit(0)
