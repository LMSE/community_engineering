...
#from pymoo.core import get_sampling, get_crossover, get_mutation
...
class DyMMMSamplesAdaptive:
    """
    Class to adaptively generate samples using a surrogate model in a multi-objective optimization context.
    
    Attributes:
        localWeight (float): Local weight.
        globalWeight (float): Global weight.
        problem (DyMMMMultiObjectiveProblem): Instance of the problem being solved.
        algorithm (NSGA2): Algorithm used for optimization.
        lowerBounds (list): Lower bounds for the parameters.
        upperBounds (list): Upper bounds for the parameters.
        scaler (MinMaxScaler): Scaler used for parameter scaling.
        paramCount (int): Number of parameters.
        surrogate (DyMMMSurrogateModel): Surrogate model used for optimization.
    """
    localWeight=0
    globalWeight=1
    problem=None
    ...
    def __init__(self, paramCount, lowerBounds, upperBounds, scaler, surrogate):
        """
        Initializes the DyMMMSamplesAdaptive class with given parameters limits and surrogate model.
        ...
        Parameters:
            ...
            upperBounds (list): Upper bounds for the parameters.
            scaler (MinMaxScaler): Scaler used for parameter scaling.
            surrogate (DyMMMSurrogateModel): Surrogate model for optimization.
        """
    ...
    def sample(self, train_X, train_y, n_last=150):
        """
        Generates samples using the surrogate model and multi-objective optimization.

        Parameters:
            train_X (DataFrame): Training data features.
            train_y (Series): Training data labels.
            n_last (int): Number of last generations to consider for termination criteria.

        Returns:
            tuple: Result of the optimization, Pareto set, and Pareto front.
        """
        ...
        #termination = RobustTermination(DesignSpaceTermination(tol=tol), period=30)
        termination=DesignSpaceTermination(tol=tol)
        #termination = DesignSpaceTermination(tol=tol, n_last=min(n_last,self.paramCount*15))
        #termination = get_termination("time", "00:15:00")

        # termination = MultiObjectiveSpaceTermination(tol=0.0025,
        #                                               n_last=30,
        #                                               nth_gen=5,
        #                                               n_max_gen=None,
        #                                               n_max_evals=None)
...
def generateTrainingDF(inputDataDir):
    """
    Generates a training DataFrame from the specified input directory.

    Parameters:
        inputDataDir (str): Directory containing the input data files.

    Returns:
        tuple: Training feature DataFrame, training label Series, and index of the last file.
    """
    ...
    if 'biomass3' in train_df.columns:
        X_train = train_df.drop(['biomass3','biomass3_SS'], axis=1)
    y_train = train_df['CSI'] 
    return X_train, y_train, lastFileIndex

def generateRangesScalar(paramsRangeFile):
    """
    Generates minimum and maximum value ranges and scalers from a parameter range file.

    Parameters:
        paramsRangeFile (str): Path to the parameter range file.

    Returns:
        tuple: Minimum value range, maximum value range, list of scalers, and the parameters range DataFrame.
    """
...
def generateSurrogate(X_train, y_train, scaler):
    """
    Generates a surrogate model based on the training data and scaler.

    Parameters:
        X_train (DataFrame): Training feature data.
        y_train (Series): Training label data.
        scaler (list[MinMaxScaler]): List of scalers for each feature.

    Returns:
        DyMMMSurrogateModel: Trained surrogate model.
    """
...
""" def generateSurrogate(X_train, y_train, scaler):
    surrogate = DyMMMSurrogateModel(X_train.shape[1])
    print(X_train.columns.tolist())
    X_train_n=X_train.drop('Parameter', axis=1).to_numpy()
    print(X_train)
    print(X_train_n)
    for i in range(X_train.shape[1]):
        if i==0:
