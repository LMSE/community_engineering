...
def plot1(dataFrame, titleText, filePath=None):
    """
    Plots multiple time series data from the DataFrame or a CSV file. The plot consists of eight subplots, each showing different variables.

    Args:
        dataFrame (DataFrame): A DataFrame containing the time series data to plot.
        titleText (str): The title of the plot.
        filePath (str, optional): If provided, the data is read from this CSV file instead of the dataFrame. Defaults to None.
...
def plot2(titleText, filePath=None):
    """
    Creates a plot for each column in the CSV file, stacking them vertically. Uses different line styles and colors for each plot.
...
def plotComplexVector(z):
    """
    Plots a complex vector, showing the real part on the X-axis and the imaginary part on the Y-axis.

    Args:
        z (list): A list of complex numbers to plot.
...
def showViolinPlot2(CSIDF, columnNames, Ylabel):
    """
    Displays a violin plot for the specified column names from the DataFrame.

    Args:
        CSIDF (DataFrame): DataFrame containing the data to plot.
        columnNames (list): List of column names to use in the plot.
...
def plotViolin(columnName='CSI', Ylabel='Diversity', titletext='diversity'):
    """
    Creates a violin plot for the specified column across different community types.

    Args:
        columnName (str, optional): Column name to plot. Defaults to 'CSI'.
        Ylabel (str, optional): Y-axis label. Defaults to 'Diversity'.
        titletext (str, optional): Plot title. Defaults to 'diversity'.
...
def plotViolin_1(columnName='CSI', Ylabel='Diversity'):
    """
    Creates a violin plot for the specified column, adjusting the glucose consumption and showing the difference in biomass.
...
def showScatterMatrix(CSIDF, rangeDF=None):
    ...
    """
    Displays a scatter matrix plot for the DataFrame, optionally filtering the data based on a range DataFrame.

    Args:
        CSIDF (DataFrame): DataFrame to plot.
        rangeDF (DataFrame, optional): DataFrame specifying the range of values to include from CSIDF.
...
def plotHyperVolume():
    """
    Plots the top 100 normalized hypervolumes for different community types.
...
def plotComplexNumbers(communitiesDir, communityName, title, index, highCSI):
    """
    Plots complex numbers from a file, filtering based on CSI values.

    Args:
        communitiesDir (str): Directory containing the community data.
        communityName (str): Name of the community.
        title (str): Title of the plot.
        index (int): Index of the value to plot.
        highCSI (bool): If True, filters for high CSI values.
